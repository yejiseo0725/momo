import {
  GatheringImageError,
  deleteGatheringImage,
  storeGatheringImage,
} from '@/lib/gathering-images';
import {
  ONLINE_REGION_CODE,
  getRegionCodesByKeyword,
  getRegionName,
  isRegionCode,
} from '@/lib/regions';
import { getUserDisplayNames, getUserProfiles } from '@/lib/users';
...
function addListDetailsToGathering(gathering, memberCounts, creatorProfiles) {
  const serializedGathering = serializeGathering(gathering);

  delete serializedGathering.inviteToken;
  const creatorProfile = creatorProfiles.get(serializedGathering.userId);

  return {
    ...serializedGathering,
    memberCount: memberCounts.get(serializedGathering.id) || 0,
    creatorName:
      typeof creatorProfile === 'string'
        ? creatorProfile
        : creatorProfile?.displayName || '알 수 없는 사용자',
    creatorImage:
      typeof creatorProfile === 'object' && creatorProfile?.image
...
export async function getPublicGatherings({
  ...
  if (keyword) {
    ...
    if (regionCodes.length > 0) {
      keywordConditions.push({ region: { $in: regionCodes } });
    }

    searchConditions.push({ $or: keywordConditions });
  }

  if (isRegionCode(nearbyRegionCode)) {
    const nearbyRegionConditions = [{ region: ONLINE_REGION_CODE }];

    if (/^\d{10}$/.test(nearbyRegionCode)) {
      const sigunguCode = nearbyRegionCode.slice(0, 5);
      nearbyRegionConditions.unshift({
        region: { $regex: `^${sigunguCode}\\d{5}$` },
      });
    }

    searchConditions.push({ $or: nearbyRegionConditions });
  }

  if (category) {
    if (Array.isArray(category) && category.length > 0) {
      query.category = category.length === 1 ? category[0] : { $in: category };
  ...
  const [memberCounts, creatorProfiles] = await Promise.all([
    getMemberCounts(database, gatheringIds),
    getUserProfiles(
      database,
      gatherings.map((gathering) => gathering.userId),
    ),
  ]);

  return gatherings.map((gathering) =>
    addListDetailsToGathering(gathering, memberCounts, creatorProfiles),
  );
}
...
export async function getGatheringDetails(gatheringId, currentUserId) {
  ...
  return {
    ...
    members: members.map((member) => ({
      ...
      displayName:
        userProfiles.get(member.userId)?.displayName || '알 수 없는 사용자',
      image: userProfiles.get(member.userId)?.image || '',
    })),
  };
}
...
export async function getGatheringInvitation(inviteToken, currentUserId) {
  if (
    ...
    inviteToken.length < 32 ||
    inviteToken.length > 100
  ) {
    return null;
  }
...
export async function createGathering(userId, input) {
  ...
  try {
    imageUrl = await storeGatheringImage(input.imageFile, { userId });
  } catch (error) {
    if (error instanceof GatheringImageError) {
      throw new GatheringError(error.message);
    }
    throw error;
  }
  ...
  try {
    ...
    await deleteGatheringImage(imageUrl);
...
export async function updateGathering(gatheringId, userId, input) {
  ...
  const gathering = await database
    ...
    .findOne(
      { _id: objectId, userId },
      { projection: { imageUrl: 1, inviteToken: 1 } },
    );
  ...
  try {
    newImageUrl = await storeGatheringImage(input.imageFile, {
      gatheringId,
      userId,
    });
  } catch (error) {
  ...
  try {
    result = await database
      .collection('gatherings')
      .updateOne({ _id: objectId, userId }, { $set: updateFields });
  } catch (error) {
    await deleteGatheringImage(newImageUrl);
    throw error;
  }
  ...
  if (newImageUrl) {
    await deleteGatheringImage(gathering.imageUrl);
  }
}
...
export async function leaveGathering(gatheringId, userId) {
  ...
  await database
    ...
    .deleteMany({ gatheringId, userId });
  await database
    .collection('gatheringMembers')
    .deleteOne({ gatheringId, userId });
}

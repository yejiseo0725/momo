...
async function getUserNames(database, userIds) {
...
function addCountToGathering(gathering, memberCounts) {
...
export async function getPublicGatherings({ keyword = "", category = "", limit = 50 } = {}) {
  ...
  const memberCounts = await getMemberCounts(database, gatheringIds);

  return gatherings.map((gathering) => addCountToGathering(gathering, memberCounts));
}
...
export async function getJoinedGatherings(userId, limit = 50) {
  ...
  return memberships
    .map((membership) => {
      ...
      return {
        ...addCountToGathering(gathering, memberCounts),
...
export async function getGatheringDetails(gatheringId, currentUserId) {
  ...
  const members = await database
    ...
    .toArray();
  const userNames = await getUserNames(database, members.map((member) => member.userId));
  const membership = members.find((member) => member.userId === currentUserId) || null;

  return {
    gathering: {
      ...serializeDocument(gathering),
...
export async function createGathering(userId, input) {
  ...
  const result = await database.collection("gatherings").insertOne(gathering);

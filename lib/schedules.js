...
export async function getSchedules(gatheringId, userId) {
  ...
  return schedules.map((schedule) => {
    const profile = profiles.get(schedule.userId);
    return {
...
export async function getScheduleDetails(scheduleId, gatheringId, userId) {
  ...
  const scheduleProfile = profiles.get(schedule.userId);

  return {
    schedule: {
      ...serializeDocument(schedule),
      authorName: scheduleProfile?.displayName || "알 수 없는 사용자",
      authorImage: scheduleProfile?.image || "",
    },
    participants: participants.map((participant) => {
      const participantProfile = profiles.get(participant.userId);
      return {
        ...serializeDocument(participant),
        displayName: participantProfile?.displayName || "알 수 없는 사용자",

...
export async function getChallenges(gatheringId, userId) {
  ...
  for (const feed of feeds) {
    ...
    const profile = profiles.get(feed.userId);
    currentFeeds.push({
  ...
  return challenges.map((challenge) => {
    ...
    const challengeFeeds = feedsByChallenge.get(serializedChallenge.id) || [];
    const profile = profiles.get(challenge.userId);
    return {
      ...serializedChallenge,
      authorName: profile?.displayName || "알 수 없는 사용자",
      authorImage: profile?.image || "",
      feedCount: challengeFeeds.length,
      feeds: challengeFeeds,
    };
  });
}

export async function getChallengeDetails(challengeId, gatheringId, userId) {
  await requireGatheringMember(gatheringId, userId);
  const objectId = parseObjectId(challengeId);
  if (!objectId) {
    return null;
  ...
  const profiles = await getUserProfiles(
    database,
    [challenge.userId, ...feeds.map((feed) => feed.userId)],
  );
  const challengeProfile = profiles.get(challenge.userId);

  return {
    challenge: {
      ...serializeDocument(challenge),
      authorName: challengeProfile?.displayName || "알 수 없는 사용자",
      authorImage: challengeProfile?.image || "",
    },
    feeds: feeds.map((feed) => {
      const feedProfile = profiles.get(feed.userId);
      return {
        ...serializeDocument(feed),
        authorName: feedProfile?.displayName || "알 수 없는 사용자",
        authorImage: feedProfile?.image || "",
      };
    }),
  };
}

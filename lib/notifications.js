...
export async function createGatheringNotifications({
  ...
  const recipientObjectIds = members
    .map((member) => parseObjectId(member.userId))
    .filter(Boolean);
  const notificationEnabledUsers = await database.collection("users").find(
    {
  ...
  const notificationEnabledUserIds = new Set(
    notificationEnabledUsers.map((user) => user._id.toString()),
  );
  const recipients = members.filter((member) => (
    notificationEnabledUserIds.has(member.userId)
  ));
  ...
  return notifications.map((notification) => {
    ...
    return {
      ...
      href: notification.type === "SCHEDULE_CREATED"
        ...
        : `/gatherings/${notification.gatheringId}/challenges/${notification.targetId}`,
    };
  });
}
